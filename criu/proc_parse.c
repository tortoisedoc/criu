		...
		int (*cb)(union fdinfo_entries *e, void *arg), void *arg)
{
	...
	while (1) {
		...
		if (fdinfo_field(str, "fanotify ino")) {
			...
			buf = xmalloc(sizeof(FanotifyMarkEntry) + sizeof(FanotifyInodeMarkEntry) +
		...
		if (fdinfo_field(str, "fanotify mnt_id")) {
			...
			buf = xmalloc(sizeof(FanotifyMarkEntry) + sizeof(FanotifyMountMarkEntry));
			if (!buf)
				goto out;
		...
		if (fdinfo_field(str, "inotify wd")) {
			...
			buf = xmalloc(sizeof(InotifyWdEntry) + sizeof(FhEntry) +
					FH_ENTRY_SIZES__min_entries * sizeof(uint64_t));
			if (!buf)
				goto out;
